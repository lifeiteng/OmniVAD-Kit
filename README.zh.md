...
## 流式分段 Streaming Segmenter — 在线产出长音频段

非流式的 `OmniVAD.detect()` 一次性跑完整的 7 步后处理；但对于长音频
（直播流、几小时录音、实时字幕），你希望**段一确认就立即拿到**，无需缓存全量信号。

`OmniStreamingVAD` 解决这个：喂任意长度 PCM chunk，段一关闭就吐 `[start, end]`。
...
流式算法实现的是 batch pipeline 的**因果子集**（step 1-4 + 7）。
Step 5/6（`merge_silence_frames` / `extend_speech_frames`）需要无界 lookahead，
传入 > 0 会显式返回 `OMNI_ERR_INVALID_ARG`。
...
for chunk in audio_chunks:           # 160-sample int16 chunks
    ...
    if result is None:
        continue
    print(f"frame {result.frame_offset}: confidence={result.confidence:.2f}")  # 逐帧
    for start, end in segmenter.process_frame(result.confidence):              # 段
        print(f"speech: {start:.2f}s -> {end:.2f}s")
...
for (const chunk of pcmChunks) {            // Int16Array，任意长度
...
如果你想在 speech 一确认就启动下游 ASR 模型（不等段关闭），每次 `process()` 后
轮询 `is_in_speech` / `active_start`：
...
`OmniStreamingVAD` 输出的是原始 VAD 段。如果想打包成 Whisper 的 30s chunk，
增量喂给 `merge_chunks`（或在流结束时批量调用）。
