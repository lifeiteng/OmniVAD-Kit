...
/* -------------------------------------------------------------------------- */
/*  2b. Stream Segmenter (frame-prob -> speech segments, online state machine)*/
/* -------------------------------------------------------------------------- */

/**
 * Pure-algorithm streaming segmenter that converts per-frame VAD probabilities
 * into [start, end] speech segments online. Does NOT load any model — pair it
 * with omni_stream_vad_process() (or any other source of per-frame probs).
 *
 * Implements the causal subset of omni_vad_detect()'s post-processing:
 *   Step 1: causal moving-average smoothing (window = smooth_window_size)
 *   Step 2: binary threshold
 *   Step 3: 4-state state machine (POSSIBLE_SPEECH waits min_speech_frames
 *           before confirming START; POSSIBLE_SILENCE waits min_silence_frames
 *           before emitting END)
 *   Step 4: smooth window start fix (extends confirmed start backward by
 *           smooth_window_size frames)
 *   Step 7: long-segment force-split at lowest-probability point when an
 *           active segment exceeds max_speech_frames
 *
 * Steps 5 (merge_silence) and 6 (extend_speech) are NOT supported because
 * they require unbounded lookahead and break the streaming O(1) invariant.
 * Passing a config with merge_silence_frames > 0 or extend_speech_frames > 0
 * makes omni_stream_segmenter_create() fail with OMNI_ERR_INVALID_ARG.
 *
 * Latency budget under default OmniPostConfig:
 *   - SEGMENT_END emission delay: ~min_silence_frames frames (~200ms)
 *   - SEGMENT confirmation delay: ~min_speech_frames frames (~200ms)
 *   - max_speech_frames force-split lookahead: max_speech_frames/2 frames
 *     (~15s under default 3000-frame cap; rarely triggers in practice)
 *
 * Thread-safe: NO. Each handle is a single-threaded state machine.
 */

/** Opaque handle for streaming segmenter. */
typedef struct OmniStreamSegmenterCtx* OmniStreamSegmenterHandle;

/**
 * Create a streaming segmenter from a post-processing config.
 *
 * @param config     post-processing config; merge_silence_frames and
 *                   extend_speech_frames must be 0 (otherwise INVALID_ARG)
 * @param out_error  receives OMNI_OK or error code on failure
 * @return handle, or NULL on failure
 */
...
/**
 * Push one frame's raw probability. Most common path — pair with
 * omni_stream_vad_process() one frame at a time.
 *
 * Output buffer is malloc'd by the library; caller must free via omni_free().
 * If no segment is emitted in this call, *out_count = 0 and *out_segments
 * is set to NULL (still safe to pass to omni_free).
 *
 * @param seg            segmenter handle
 * @param prob           raw model output probability for the next frame [0, 1]
 * @param out_segments   receives malloc'd array of completed segments (or NULL)
 * @param out_count      receives number of segments emitted (typically 0 or 1)
...
/**
 * Push a batch of probabilities. Convenience wrapper for tests (oracle
 * equivalence) and for callers that already have a probs buffer.
 */
...
/**
 * Flush the in-progress segment (if any) at end-of-stream.
 *
 * Tail segment end-time follows whole-audio convention:
 *   end = num_frames * 0.01 + 0.025 (frame end of last accepted frame)
 * If total_samples_seen > 0, the end time is clamped to
 *   total_samples_seen / 16000.0
 * Pass total_samples_seen = 0 to skip clamping.
 *
 * @param seg                 segmenter handle
 * @param total_samples_seen  total int16 samples fed to upstream (0 to skip clamp)
 * @param out_segments        receives 0 or 1 segment
 * @param out_count           receives 0 or 1
 */
...
/** Whether the segmenter is currently in a confirmed speech segment. */
OMNIVAD_API bool omni_stream_segmenter_is_in_speech(OmniStreamSegmenterHandle seg);

/**
 * If in confirmed speech, return the start time (seconds) of the active
 * segment. Returns -1.0f if not in speech.
 */
OMNIVAD_API float omni_stream_segmenter_get_active_start(OmniStreamSegmenterHandle seg);

/** Reset all internal state (state machine, buffers, frame counter). */
